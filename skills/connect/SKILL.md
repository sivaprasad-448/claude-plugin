...
3. Remove any existing Lightup MCP config first:
   ```
   claude mcp remove lightup -s user
   ```
   (ignore errors if it doesn't exist)

4. Run claude mcp add with the extracted values:
   ```
   claude mcp add --transport sse lightup "<mcp-url>/sse?host=<data.server>&refresh_token=<data.refresh>" -s user
   ```

5. Tell the user: "Connected! You can now ask questions about your data. Try: 'What metrics do I have?' or 'Show me recent incidents'."
